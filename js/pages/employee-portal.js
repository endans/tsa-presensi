...
async function initEmpPortal() {
  ...
  document.getElementById('empGreeting').textContent = `${greet}, ${emp.nama}! Semoga hari Anda menyenangkan.`;
  ...
  if (sData) {
    ...
    settings.toleransi = sData.toleransi;
  }
...
function renderEmpHistory() {
  ...
  if (summary) {
    ...
    const workDays  = 26;
    const pct       = Math.min(100, Math.round(hadir/workDays*100));
    summary.innerHTML = `
...
function checkAndConfirmEmp() {
  ...
  const isLate = empCurrentAbsenType==='masuk' &&
    (now.getHours()>h || (now.getHours()===h && now.getMinutes()>m+settings.toleransi));
  ...
  try {
    ...
    if (empCurrentAbsenType === 'masuk') {
      ...
      const { error } = await sb.from('absensi')
        ...
        .eq('employee_id', emp.id).eq('tanggal', dateStr);
      if (error) throw error;
